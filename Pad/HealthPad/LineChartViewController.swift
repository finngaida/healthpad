...
public class LineChartViewController: UIViewController {
    ...
    public override func viewDidLoad() {
        ...
        heartRateView = HeartRateView(frame: CGRectMake(50, (height + 20) * 0 + off, width, height))
        scrollView.addSubview(heartRateView!)
        
        bloodPressureView = BloodPressureView(frame: CGRectMake(50, (height + 20) * 1 + off, width, height))
        scrollView.addSubview(bloodPressureView!)
        
        sleepView = SleepView(frame: CGRectMake(50, (height + 20) * 2 + off, width, height))
        scrollView.addSubview(sleepView!)
        
        stepsView = StepsView(frame: CGRectMake(50, (height + 20) * 3 + off, width, height))
        scrollView.addSubview(stepsView!)
        
        energyView = ActiveEnergyView(frame: CGRectMake(50, (height + 20) * 4 + off, width, height))
        scrollView.addSubview(energyView!)
        
        weightView = WeightView(frame: CGRectMake(50, (height + 20) * 5 + off, width, height))
...
//        weightView?.setData([79.5, 79.0, 80.0, 79.0, 81.0, 80.5, 79.0, 70.5].map({Weight(value:$0, description:"", unit:Unit.kg, date:nil)}))
        ...
        if let data = Helper.sharedHelper.latestData {
            ...
            if let weights = data["Weight"] {
                ...
                weightView?.setData(weights.map({Weight(value:Double("\(($0 as! GeneralHealthObject).value)") ?? 0, description:"\($0)", unit:Unit.kg, date:nil)}))
